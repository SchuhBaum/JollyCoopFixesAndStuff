...
{
    [BepInPlugin("SchuhBaum.JollyCoopFixesAndStuff", "JollyCoopFixesAndStuff", "1.24")]
    public class MainMod : BaseUnityPlugin
    {
        ...
        private void RainWorld_Start(On.RainWorld.orig_Start orig, RainWorld rainWorld)
        {
            ...
            foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                // using string.Contains() creates ambiguity
                // at least in the case of JollyCoop;
                string name = assembly.GetName().Name;
                if (name == "FancySlugcats" || name == "FancySlugcats;;0" || name == "CustomTail" || name == "CustomTail;;0")
                {
                    matchCharacterWithPlayerNumber = true;
                }
                else if (name == "JollyCoop" || name == "JollyCoop;;0")
                {
                    isJollyCoopEnabled = true;
                }
                else if (name == "ShelterBehaviors" || name == "ShelterBehaviors;;0")
                {
                    isShelterBehaviorsEnabled = true;
                }
                else if (name == "SBCameraScroll" || name == "SBCameraScroll;;0")
                {
                    isSBCameraScrollEnabled = true;
                }
                else if (name == "SplitScreenMod" || name == "SplitScreenMod;;0")
                {
            ...
            else
            {
                ...
                SocialEventRecognizerMod.OnEnable();

                SocialMemoryMod.OnEnable();
                SpearMod.OnEnable();
